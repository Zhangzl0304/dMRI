...
def CMR_sampling_major_vote_func(batch_size, diffusion_model, ddp_model, output_folder, dataset, logger, clip_denoised):
    ddp_model.eval()
      ...
      for data_idx, data in enumerate(loader):
          ...
          for i in range(gt_mask.shape[0]):
              ...
              gt_img.convert("L").save(
                  os.path.join(output_folder, f"gt_{data_idx}_{i}.png"))
          ...
          for i in range(condition_on.shape[0]):
              ...
              in_img.convert("L").save(
                  os.path.join(output_folder, f"input_{data_idx}_{i}.png"))
          ...
          for round_index in range(n_rounds):
            ...
            pred_cur = x.cpu()
            print(np.array(gt_mask).ndim, np.array(pred_cur).ndim)
            psnr_one = psnr(np.array(gt_mask)/np.array(gt_mask).max(), np.array(pred_cur)/np.array(pred_cur).max())
            base_psnr = psnr(np.array(gt_mask)/np.array(gt_mask).max(), np.array(im_und.cpu())/np.array(im_und.cpu()).max())
            ssim_one = ssim(np.array(gt_mask)/np.array(gt_mask).max(), np.array(pred_cur)/np.array(pred_cur).max())
            base_ssim = ssim(np.array(gt_mask)/np.array(gt_mask).max(), np.array(im_und.cpu())/np.array(im_und.cpu()).max())
            print('one round PSNR:',psnr_one)
            print('base PSNR:', base_psnr)
            print('one round SSIM:',ssim_one)
            print('base SSIM:', base_ssim)
            logger.info(f'one round PSNR: {psnr_one}')
            logger.info(f'base PSNR: {base_psnr}')
            x_pred = x_pred + x.cpu()
            print(gt_mask.shape, x_pred.shape,'shapppee')

            for i in range(x.shape[0]):
                out_img = Image.fromarray((x[i][0].detach().cpu().numpy())*255)
                out_img.convert("L").save(
                    os.path.join(output_folder, f"model_output_for_the_{data_idx}th_sample_in_{round_index}round_of_{i}frame.png"))

          x_pred_avg = x_pred/n_rounds
          psnr_ = psnr(np.array(gt_mask)/np.array(gt_mask).max(), np.array(x_pred_avg) / np.array(x_pred_avg).max())
          print(psnr_,'avgpsnr')
          mse_ = mse(np.array(gt_mask)/np.array(gt_mask).max(), np.array(x_pred_avg) / np.array(x_pred_avg).max())
          nmse_ = nmse(np.array(gt_mask)/np.array(gt_mask).max(), np.array(x_pred_avg) / np.array(x_pred_avg).max())
          ssim_ = ssim(np.array(gt_mask)/np.array(gt_mask).max(), np.array(x_pred_avg) / np.array(x_pred_avg).max())
          ...
          # for index, (gt_im, out_im) in enumerate(zip(gt_mask.cpu(), x_pred_avg)):
          #       gt_im = gt_im.cpu().squeeze(1).squeeze(1).detach().numpy()
          #       out_im = out_im.cpu().squeeze(1).squeeze(1).detach().numpy()
          #
          #       # H, W = gt_im.shape[-2], gt_im.shape[-1]
          #       # gt_im = gt_im[:, H//4:H//4*3, W//3:W//3*2]
          #       # out_im = out_im[:, H//4:H//4*3, W//3:W//3*2]
          #       psnr_ = psnr(gt_im/gt_im.max(), out_im/out_im.max())
          #       mse_ = mse(gt_im/gt_im.max(), out_im/out_im.max())
          #       nmse_ = nmse(gt_im/gt_im.max(), out_im/out_im.max())
          #       ssim_ = ssim(gt_im/gt_im.max(), out_im/out_im.max())
          #       psnr_list.append(psnr_)
          #       mse_list.append(mse_)
          #       nmse_list.append(nmse_)
          #       ssim_list.append(ssim_)
    ...
    # psnr_tensor = torch.tensor(psnr_list + [torch.tensor(-1)], device=dist_util.dev())
    # ssim_tensor = torch.tensor(ssim_list + [torch.tensor(-1)], device=dist_util.dev())
    # mse_tensor = torch.tensor(mse_list + [torch.tensor(-1)], device=dist_util.dev())
    # nmse_tensor = torch.tensor(nmse_list + [torch.tensor(-1)] , device=dist_util.dev())
